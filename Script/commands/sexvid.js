module.exports.config = {
  name: "sexvid",
  version: "2.0",
  hasPermssion: 0, // মিরাইতে role এর বদলে এটা ব্যবহার হয় (০ = সবাই)
  credits: "nexo_here",
  description: "পর্ন ভিডিও দেখার কমান্ড",
  commandCategory: "18+",
  usages: "sexvid",
  cooldowns: 30, // কাউন্টডাউন সেকেন্ডে
};

// আগে পাঠানো ভিডিও ট্র্যাক করার জন্য
if (!global.sentVideos) {
  global.sentVideos = [];
}
...
module.exports.run = async ({ api, event, args }) => {
  ...
  // লোডিং মেসেজ পাঠানো
  const loadingMsg = await api.sendMessage("Tham video dicchi ektu Dara 😏", threadID, messageID);

  // ডুপ্লিকেট ভিডিও এড়ানো
  let availableVideos = links.filter(v => !global.sentVideos.includes(v));
  if (availableVideos.length === 0) {
    global.sentVideos = [];
    availableVideos = links;
  }

  const randomVideo = availableVideos[Math.floor(Math.random() * availableVideos.length)];
  global.sentVideos.push(randomVideo);

  const path = __dirname + `/cache/sexvid_${senderID}.mp4`;
  ...
  try {
    ...
    await api.sendMessage({
      ...
      fs.unlinkSync(path);
      // ৫০ সেকেন্ড পর লোডিং মেসেজ ডিলিট করা
      setTimeout(() => api.unsendMessage(loadingMsg.messageID), 50000);
    }, messageID);

  } catch (e) {
    return api.sendMessage(`Error: ভিডিও পাঠাতে সমস্যা হয়েছে!`, threadID, messageID);
  }
};
